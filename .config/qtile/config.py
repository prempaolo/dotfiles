...
def toggle_telegram(qtile):
    # Safely grab the windows map
    win_dict = getattr(qtile, "windows_map", {})

    for window in win_dict.values():
        try:
            wm_class = window.get_wm_class()
            if wm_class:
                if any("telegram" in str(c).lower() for c in wm_class):
                    window.kill()
                    return
        except Exception as e:
            # Catching internal objects that don't support get_wm_class()
            pass

    # If no Telegram windows is open it opens it
    qtile.spawn("/usr/sbin/Telegram")
...
keys = [
    ...
    Key([mod], "minus", lazy.function(toggle_telegram), desc="Launch Telegram"),
    Key(
        [],
...
floating_layout = layout.Floating(
    ...
    float_rules=[
        ...
        *layout.Floating.default_float_rules,
        ...
        Match(wm_class="TelegramDesktop"), # Telegram 
        Match(wm_class="Spotify"), # Telegram 
        Match(title=re.compile(r"^Friends List$")), # Steam Friends Chat
    ],
)
